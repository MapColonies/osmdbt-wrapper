...
export const osmdbtProcessorFactory: FactoryFunction<OsmdbtProcessor> = (container: DependencyContainer) => {
  ...
  scheduledTask.on('task:started', (ctx) => logger.debug({ msg: 'scheduled task fired task:started event', ctx }));
  scheduledTask.on('task:stopped', (ctx) => logger.debug({ msg: 'scheduled task fired task:stopped event', ctx }));
  scheduledTask.on('task:destroyed', (ctx) => logger.debug({ msg: 'scheduled task fired task:destroyed event', ctx }));
  scheduledTask.on('execution:started', (ctx) => logger.debug({ msg: 'scheduled task fired execution:started event', ctx }));
  scheduledTask.on('execution:failed', (ctx) => logger.debug({ msg: 'scheduled task fired execution:failed event', ctx }));
  scheduledTask.on('execution:finished', (ctx) => logger.debug({ msg: 'scheduled task fired execution:finished event', ctx }));
  scheduledTask.on('execution:missed', (ctx) => logger.warn({ msg: 'scheduled task fired execution:missed event', ctx }));
  scheduledTask.on('execution:overlap', (ctx) => logger.warn({ msg: 'scheduled task fired execution:overlap event', ctx }));
